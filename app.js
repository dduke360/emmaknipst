...
function generateCaseStudiesFromPhotos(photos) {
  ...
  return Array.from(byCategory.entries())
    .map(([categoryId, group]) => {
      ...
      const photoIds = sorted.slice(0, 3).map((photo) => photo.id);
  ...
  const normalized = rawCaseStudies
    .map((study, index) => {
      ...
      return {
        ...
        photoIds: photoIds.length ? photoIds : [featuredPhotoId],
        featuredPhotoId,
...
function renderCaseStudyDetail(activeSlug = '') {
  ...
  if (!section || !kicker || !title || !intro || !metrics || !story || !hero || !grid || !backLink) return;
